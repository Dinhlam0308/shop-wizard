...
class MenuController extends Controller
{
    ...
    {
        try {
            //Lộc menu match với category nếu có
            ...
            $categories = Menu::query()->select('category')
                ->whereNotNull('category')
                ->distinct()
                ->pluck('category');
            return view('user.menu.index', compact('menu', 'categories'));
        } catch (\Exception $e) {
        ...
        try {
            ...
            $validator = Validator::make($request->all(), [
                ...
                'is_special' => [
                    'nullable',
                    'boolean',
                    Rule::unique('menus', 'is_special')
                        ->ignore($menu->id)
                        ->where(fn($query) => $query->where('is_special', true)),
                ],
                'available' => 'nullable|boolean',
                'image' => 'nullable|image|mimes:jpeg,png,jpg,gif|max:5120',
            ]);
