<x-app-layout>
    ...
    <x-slot name="header">
        <div class="sr-only">
            <h1>Cửa hàng Cho thuê Phép thuật</h1>
        </div>
        {{-- Tiêu đề --}}
        <header class="text-center mb-4 space-y-3">
            <h2
                ...
                <i data-lucide="key" class="w-8 h-8 text-amber-400 animate-pulse"></i>
                <span data-vi="Cửa hàng Cho thuê" data-en="Rental Shop"></span>
                <i data-lucide="stars" class="w-8 h-8 text-yellow-300 animate-pulse"></i>
            </h2>
            <p class="mt-4 text-gray-300 max-w-2xl mx-auto text-lg leading-relaxed">
                <span data-vi="Bộ sưu tập cho thuê – sang trọng và độc quyền."
                    data-en="Rental collection – luxurious and exclusive."></span>
            </p>
        </header>
    </x-slot>

    {{-- === Khu vực chính (Magic Glass Section) === --}}
    <section class="relative max-w-7xl mx-auto px-6 font-[Inter]">

        {{-- Danh mục sản phẩm --}}
        <div x-data="{ showAll: false }" x-init="$nextTick(() => lucide.createIcons())" class="text-center mb-10">
            <div class="flex flex-wrap justify-center gap-3 mb-6">
                {{-- Tất cả --}}
                <a href="{{ route('user.shop.rental', ['category_id' => null]) }}"
                ...
                @foreach ($categories as $index => $cat)
                    <template x-if="showAll || {{ $index }} < 8">
                        <a href="{{ route('user.shop.rental', ['category_id' => $cat->id]) }}"
                            class="flex items-center gap-2 px-5 py-2.5 rounded-full 
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            @forelse ($products as $product)
                ...
                <article
                    ...
                    <div class="aspect-[4/3] overflow-hidden relative">
                        ...
                        <i data-lucide="key" class="absolute top-3 left-3 w-5 h-5 text-amber-300"></i>
                    </div>
                    ...
                    <div class="p-6 flex flex-col justify-between h-[240px]">
                        <div>
                            ...
                            <p class="text-gray-400 text-sm line-clamp-2">
                                {{ $product->description ?? 'Vật phẩm cao cấp dành cho thuê ngắn hạn.' }}
                            </p>
                        </div>
                        ...
                        <div class="mt-4 flex items-end justify-between">
                            <div>
                                <div class="text-xs text-gray-400">
                                    <span data-vi="Giá thuê" data-en="Rental Price"></span>
                                </div>
                                <div
                                    class="text-sm font-bold bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">
                                    {{ number_format($product->price, 0, ',', '.') }}vnd
                                    <span class="text-gray-400 font-normal" data-vi="/ngày" data-en="/day"></span>
                                </div>
                            </div>
            ...
            @empty
                <div class="col-span-full text-center text-gray-400">
                    <span data-vi="Hiện chưa có vật phẩm cho thuê." data-en="No rental items available."></span>
                </div>
            @endforelse
        </div>

        {{-- Ghi chú chỉ cho thuê tại cửa hàng (tinh tế, Lucide icon) --}}
        <div class="text-center mt-10">
            <p class="flex items-center justify-center gap-2 text-sm text-gray-400/90 italic">
                ...
                <span data-vi="Tất cả vật phẩm tại đây chỉ cho thuê trực tiếp tại cửa hàng."
                    data-en="All items here are only available for rent directly at the store."></span>
                <span class="not-italic text-gray-500/80" data-vi="Vui lòng liên hệ nhân viên để biết thêm chi tiết."
                    data-en="Please contact staff for more details."></span>
            </p>
        </div>
