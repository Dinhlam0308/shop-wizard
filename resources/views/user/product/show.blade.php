...
<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <nav class="text-sm mb-6">
                <ol class="flex flex-wrap items-center gap-2 text-gray-500 dark:text-gray-400">
                    ...
                    @else
                        <li><a class="hover:text-amber-400" href="{{ route('user.shop.accessories') }}">Accessories</a></li>
                    @endif
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8"
                 x-data="productShowPage(@js($product->image), @js(($product->images ?? collect())->pluck('image_url')))">
                ...
                <div class="lg:col-span-7">
                                ...
                                <template x-for="(img, idx) in allImages" :key="idx">
                                    <button type="button"
                                            class="relative shrink-0 w-20 h-20 rounded-2xl overflow-hidden
                                                   border transition
                                                   hover:scale-[1.02]"
                                            :class="active === img ? 'border-amber-400 ring-2 ring-amber-400/25' : 'border-gray-200/70 dark:border-gray-800/70'"
                                            @click="active = img">
                    ...
                    {{-- Description card --}}
                    <div class="mt-6 rounded-3xl border border-gray-200/70 dark:border-gray-800/70
                                bg-white/60 dark:bg-gray-900/40 backdrop-blur-xl
                ...
                <div class="lg:col-span-5">
                    <div
                        ...
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                    @if($product->category)
                                        Category: <span class="text-gray-900 dark:text-gray-100 font-medium">{{ $product->category->name }}</span>
                                    @else
                                        Category: <span class="text-gray-900 dark:text-gray-100 font-medium">Uncategorized</span>
                                    @endif
                                </p>
                            </div>
                            ...
                            <div class="flex items-end justify-between">
                                <div>
                                    <p class="text-xs text-gray-500 dark:text-gray-400 font-medium">
                                        @if($product->is_rental) Rental price @else Price @endif
                                    </p>
                        ...
                        <div class="mt-6 grid grid-cols-2 gap-3 text-sm">
                            <div class="rounded-2xl border border-gray-200/70 dark:border-gray-800/70 p-4 bg-white/60 dark:bg-gray-900/35">
                            ...
                            <div class="w-full h-full flex items-center justify-center p-4">
                                <div class="relative max-w-5xl w-full">
                                    ...
                                    <div class="rounded-3xl overflow-hidden border border-white/15 bg-black/40">
                                        <img :src="lightbox.src" class="w-full max-h-[80vh] object-contain bg-black" />
                                    </div>
                                </div>
                            </div>
                        </div>
                                        ...
                                        <a href="{{ route('user.product.show', $item->id) }}"
                                           class="group rounded-2xl overflow-hidden border border-gray-200/70 dark:border-gray-800/70
                                            ...
                                            <div class="aspect-[4/3] bg-gray-100 dark:bg-gray-900">
                                                <img src="{{ $item->image }}" class="w-full h-full object-cover group-hover:scale-[1.03] transition" />
                                            </div>
                                            <div class="p-3">
    ...
    {{-- Alpine helpers --}}
    <script>
        function productShowPage(mainImage, galleryUrls) {
            const clean = (arr) => (Array.isArray(arr) ? arr.filter(Boolean) : []);

            const all = [mainImage, ...clean(galleryUrls)];
